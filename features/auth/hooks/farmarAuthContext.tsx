...
export function FarmarAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const register = async (data: {
    ...
    try {
      ...
      // 7. Odeslat ověřovací / uvítací email (OTP magic link)
      try {
        ...
        if (otpError) {
          console.error('Chyba odeslání ověřovacího emailu:', otpError.message);
        }
      } catch (emailErr) {
        // Neblokujeme registraci pokud selže odeslání emailu
        console.error('Výjimka při odesílání ověřovacího emailu:', emailErr);
      }

      // 8. Vrátit farm_number
  ...
  const loginWithPin = async (usernameOrFarmNumber: string, password: string): Promise<{ success: boolean; error?: string; remainingAttempts?: number }> => {
    try {
      ...
      // Hledáme podle username, farm_number nebo email
      const { data: farmers, error } = await supabase
        .from('pestitele')
        .select('*')
        .or(`username.eq.${usernameOrFarmNumber},farm_number.eq.${usernameOrFarmNumber},email.eq.${usernameOrFarmNumber}`)
      ...
      // 4. Zkontrolovat ověření emailu
      const farmer = farmers[0];

      if (farmer.email_verified === false) {
        return {
          success: false,
          error: 'Nejprve ověřte svůj email. Zkontrolujte schránku a klikněte na odkaz, který jsme vám poslali při registraci.',
        };
      }

      // 5. HASHOVÁNÍ - Porovnat heslo s hashem
  ...
  const checkMagicLinkSession = async (): Promise<boolean> => {
    try {
      ...
      // Magic link = email ověřen → nastavit email_verified: true pokud ještě není
      if (!farmarData.email_verified) {
        await supabase
          .from('pestitele')
          .update({ email_verified: true })
          .eq('id', farmarData.id);
        farmarData.email_verified = true;
      }
