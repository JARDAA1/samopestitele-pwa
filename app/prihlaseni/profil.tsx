...
export default function ProfilLoginScreen() {
  ...
  const handleOdeslatMagicLink = async () => {
    ...
    if (!cleanEmail) {
      Platform.OS === 'web' ? alert('Zadejte emailovou adresu') : Alert.alert('Chyba', 'Zadejte emailovou adresu');
      return;
    }
    if (!cleanEmail.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
      Platform.OS === 'web' ? alert('Zadejte platnou emailovou adresu') : Alert.alert('Chyba', 'Zadejte platnou emailovou adresu');
      return;
    }
    ...
    if (result.success) {
      ...
      if (Platform.OS === 'web') {
        ...
        Alert.alert('Email odeslán', 'Zkontrolujte svou emailovou schránku a klikněte na odkaz pro obnovu hesla.', [{ text: 'OK' }]);
      }
    } else {
      Platform.OS === 'web' ? alert(result.error || 'Nepodařilo se odeslat email. Zkontrolujte emailovou adresu.') : Alert.alert('Chyba', result.error || 'Nepodařilo se odeslat email. Zkontrolujte emailovou adresu.');
    }
  };
        ...
        {!showMagicLink ? (
          <View style={styles.card}>
            <View style={styles.iconContainer}>
              <Image source={require('../../assets/images/profil-icon.png')} style={styles.iconImage} resizeMode="contain" />
            </View>

            <Text style={styles.title}>Přihlášení</Text>
            <Text style={styles.subtitle}>Přihlaste se pomocí uživatelského jména a hesla</Text>
            ...
            <AuthLoginForm
              ...
              onForgotPassword={() => setShowMagicLink(true)}
              forgotLabel="Zapomenuté heslo?"
            />
            ...
            <TouchableOpacity style={styles.secondaryButton} onPress={() => router.push('/registrace')}>
              <Text style={styles.secondaryButtonText}>Vytvořit účet</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <View style={styles.card}>
            ...
            {emailSent ? (
              <>
                ...
                <TouchableOpacity
                  ...
                  disabled={loading}
                >
                  <Text style={styles.primaryButtonText}>{loading ? 'Odesílám...' : 'Odeslat znovu'}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.backToLoginLink}
                  onPress={() => { setShowMagicLink(false); setEmailSent(false); setEmail(''); }}
                >
                  <Text style={styles.backToLoginText}>← Zpět na přihlášení</Text>
                </TouchableOpacity>
              </>
            ) : (
              <>
                ...
                <TouchableOpacity
                  ...
                  disabled={loading}
                >
                  <Text style={styles.primaryButtonText}>{loading ? 'Odesílám...' : 'Odeslat odkaz pro obnovu'}</Text>
                </TouchableOpacity>

                <TouchableOpacity style={styles.backToLoginLink} onPress={() => setShowMagicLink(false)}>
                  <Text style={styles.backToLoginText}>← Zpět na přihlášení</Text>
                </TouchableOpacity>
              </>
            )}
...
const styles = StyleSheet.create({
  ...
  backToLoginLink: { padding: 16, alignItems: 'center' },
  backToLoginText: { color: 'rgba(255,255,255,0.7)', fontSize: 14 },
});
